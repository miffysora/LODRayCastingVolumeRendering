...
namespace miffy{
  static const double PI = 6.0*asin( 0.5 );
	//行列の掛け算
	static void multMatrix(double matrix[], float org[4], float dst[4]) {
	float tmp[4];
...
static float getNDPlane(double* _projmatrix,float _plane)
{
	float planePos[4];
	...
	multMatrix(_projmatrix, planePos, planePos);
	
	return planePos[2]/planePos[3];
}
/// たぶん、視野角を求めると思われる。
static void getNDRad(double* _projmatrix,float* plane)
{
	float planePos[4];
	
	memcpy(planePos,plane,sizeof(float)*3);
	planePos[3] = 1.0f;
...
class frustum{
	...
	*/
	void setFromPerspective(double* _projmatrix,float _angle, float _ratio, float _nearD, float _farD){
		ratio = _ratio;
		angle = _angle;
		nearD = _nearD;
		farD = _farD;
		...
		if(NDNearPos[0] >= NDNearPos[1]){
			radious1 = NDNearPos[1]/3.0f;
			radious2 = 2.0f*NDNearPos[1]/3.0f;
		}
		else{
			radious1 = NDNearPos[0]/3.0f;
