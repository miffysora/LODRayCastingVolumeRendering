...
#ifdef MIFFY_VEC2
		vec2<T> projectedsize(const mat4<double>& _modelmatrix,const mat4<double>& _projmatrix,const vec2<int>& _winsize){
			double dis_from_camera;//カメラから頂点への距離
			double max_distance = -DBL_MAX;//カメラから頂点への距離の最大値
			
			vec4<double> local_pos;//オブジェクトの中心が原点な座標系
			vec4<double> view_pos;//視点が原点な座標系
			vec4<double> clip_pos;//クリップ座標での位置
			vec2<double> normalized_device_pos;
			...
			return vec2<T>(maxPixel-minPixel);//引き算する＝距離・サイズ
		}
#endif
